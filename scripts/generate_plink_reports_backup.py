    ...
    def discover_plots(self):
        ...
        for plot_file in plot_files:
            ...
            if len(parts) == 2:
                ...
                pheno, plot_type = parts
                if pheno not in self.pheno_summary_plots:
                    self.pheno_summary_plots[pheno] = []
                self.pheno_summary_plots[pheno].append({
                    'type': plot_type,
                    'path': plot_file
                })
            elif len(parts) >= 3:
                # GWAS result plots: {cohort}.{pheno}.{plot_type}.png
                cohort = parts[0]
                pheno = parts[1]
                plot_type = parts[2]
...
"""
    ...
    def generate_cohort_pheno_page(self, cohort, pheno):
        ...
        # Build plot section (always, regardless of hits)
        plot_content = ""
        ...
        pagination_controls = "" if cohort_pheno_data.empty else """
    ...
    def generate_phenotype_summary(self):
        """Generate the phenotype summary page showing pheno-level plots."""
        if self.pheno_summary_plots:
            sections = []
            for pheno in sorted(self.pheno_summary_plots.keys()):
                plots = self.pheno_summary_plots[pheno]
                imgs = "\n".join(
                    f'<div class="plot-wrapper"><img src="{p["path"]}" alt="{pheno} {p["type"]}" class="plot-image" onclick="toggleImageSize(this)"></div>'
                    for p in plots
                )
                sections.append(f'<div class="plot-container"><h3>{pheno}</h3><div class="plot-grid">{imgs}</div></div>')
            content = "<h2>Phenotype Summary</h2>\n" + "\n".join(sections)
        else:
            content = "<h2>Phenotype Summary</h2><p>No phenotype summary plots found.</p>"
        page = self.generate_page_template(content, "Phenotype Summary")
        (self.output_dir / "phenotype_summary.html").write_text(page)
        print(f"Generated page: {self.output_dir / 'phenotype_summary.html'}")

    def generate_method_summary(self):
        """Generate the method summary page."""
        cohorts_str = ", ".join(self.cohorts)
        phenos_str = ", ".join(self.phenotypes)
        content = f"""
        <div id="method-summary">
            <h2>PLINK 2.0 Methods Summary</h2>
            <h3>Workflow Set-Up</h3>
            <p><b>Cohorts:</b> {cohorts_str}</p>
            <p><b>Phenotypes:</b> {phenos_str}</p>
        </div>
        """
    ...
    def generate_all_reports(self):
        ...
        print("Generating phenotype summary...")
